...
export const useLocalInventory = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (itemsResult.data) setItems(itemsResult.data.data || []);
      if (categoriesResult.data) setCategories(categoriesResult.data.data || []);
      if (transactionsResult.data) setTransactions(transactionsResult.data.data || []);
      if (maintenanceResult.data) setMaintenance(maintenanceResult.data.data || []);
      if (alertsResult.data) setAlerts(alertsResult.data.data || []);
